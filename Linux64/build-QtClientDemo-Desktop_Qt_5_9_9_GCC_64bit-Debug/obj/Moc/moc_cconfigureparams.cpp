/****************************************************************************
** Meta object code from reading C++ file 'cconfigureparams.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.9.9)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../../src/ParaConfig/configure_params/cconfigureparams.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'cconfigureparams.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.9.9. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_CConfigureParams_t {
    QByteArrayData data[9];
    char stringdata0[134];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_CConfigureParams_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_CConfigureParams_t qt_meta_stringdata_CConfigureParams = {
    {
QT_MOC_LITERAL(0, 0, 16), // "CConfigureParams"
QT_MOC_LITERAL(1, 17, 16), // "loadRemoteParams"
QT_MOC_LITERAL(2, 34, 0), // ""
QT_MOC_LITERAL(3, 35, 22), // "on_btnIPParams_clicked"
QT_MOC_LITERAL(4, 58, 28), // "on_btnHardDiskParams_clicked"
QT_MOC_LITERAL(5, 87, 12), // "GetDVRUserID"
QT_MOC_LITERAL(6, 100, 12), // "SetDVRUserID"
QT_MOC_LITERAL(7, 113, 7), // "lUserID"
QT_MOC_LITERAL(8, 121, 12) // "showLoginBtn"

    },
    "CConfigureParams\0loadRemoteParams\0\0"
    "on_btnIPParams_clicked\0"
    "on_btnHardDiskParams_clicked\0GetDVRUserID\0"
    "SetDVRUserID\0lUserID\0showLoginBtn"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_CConfigureParams[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       6,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    0,   44,    2, 0x0a /* Public */,
       3,    0,   45,    2, 0x0a /* Public */,
       4,    0,   46,    2, 0x0a /* Public */,
       5,    0,   47,    2, 0x0a /* Public */,
       6,    1,   48,    2, 0x0a /* Public */,
       8,    0,   51,    2, 0x0a /* Public */,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void, QMetaType::Long,    7,
    QMetaType::Void,

       0        // eod
};

void CConfigureParams::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        CConfigureParams *_t = static_cast<CConfigureParams *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->loadRemoteParams(); break;
        case 1: _t->on_btnIPParams_clicked(); break;
        case 2: _t->on_btnHardDiskParams_clicked(); break;
        case 3: _t->GetDVRUserID(); break;
        case 4: _t->SetDVRUserID((*reinterpret_cast< long(*)>(_a[1]))); break;
        case 5: _t->showLoginBtn(); break;
        default: ;
        }
    }
}

const QMetaObject CConfigureParams::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_CConfigureParams.data,
      qt_meta_data_CConfigureParams,  qt_static_metacall, nullptr, nullptr}
};


const QMetaObject *CConfigureParams::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *CConfigureParams::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_CConfigureParams.stringdata0))
        return static_cast<void*>(this);
    return QWidget::qt_metacast(_clname);
}

int CConfigureParams::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 6)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 6;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 6)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 6;
    }
    return _id;
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
