/****************************************************************************
** Meta object code from reading C++ file 'cdlgremoterecordschedule.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.9.9)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../../src/ParaConfig/configure_params/RemoteParams/ChannelParams/cdlgremoterecordschedule.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'cdlgremoterecordschedule.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.9.9. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_CDlgRemoteRecordSchedule_t {
    QByteArrayData data[7];
    char stringdata0[114];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_CDlgRemoteRecordSchedule_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_CDlgRemoteRecordSchedule_t qt_meta_stringdata_CDlgRemoteRecordSchedule = {
    {
QT_MOC_LITERAL(0, 0, 24), // "CDlgRemoteRecordSchedule"
QT_MOC_LITERAL(1, 25, 21), // "on_btnCopyDay_clicked"
QT_MOC_LITERAL(2, 47, 0), // ""
QT_MOC_LITERAL(3, 48, 18), // "on_btnSave_clicked"
QT_MOC_LITERAL(4, 67, 19), // "comboWeekDayChanged"
QT_MOC_LITERAL(5, 87, 8), // "iWeekDay"
QT_MOC_LITERAL(6, 96, 17) // "clickRecordAllDay"

    },
    "CDlgRemoteRecordSchedule\0on_btnCopyDay_clicked\0"
    "\0on_btnSave_clicked\0comboWeekDayChanged\0"
    "iWeekDay\0clickRecordAllDay"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_CDlgRemoteRecordSchedule[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    0,   34,    2, 0x0a /* Public */,
       3,    0,   35,    2, 0x0a /* Public */,
       4,    1,   36,    2, 0x0a /* Public */,
       6,    0,   39,    2, 0x0a /* Public */,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Int, QMetaType::Int,    5,
    QMetaType::Int,

       0        // eod
};

void CDlgRemoteRecordSchedule::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        CDlgRemoteRecordSchedule *_t = static_cast<CDlgRemoteRecordSchedule *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->on_btnCopyDay_clicked(); break;
        case 1: _t->on_btnSave_clicked(); break;
        case 2: { int _r = _t->comboWeekDayChanged((*reinterpret_cast< int(*)>(_a[1])));
            if (_a[0]) *reinterpret_cast< int*>(_a[0]) = std::move(_r); }  break;
        case 3: { int _r = _t->clickRecordAllDay();
            if (_a[0]) *reinterpret_cast< int*>(_a[0]) = std::move(_r); }  break;
        default: ;
        }
    }
}

const QMetaObject CDlgRemoteRecordSchedule::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_CDlgRemoteRecordSchedule.data,
      qt_meta_data_CDlgRemoteRecordSchedule,  qt_static_metacall, nullptr, nullptr}
};


const QMetaObject *CDlgRemoteRecordSchedule::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *CDlgRemoteRecordSchedule::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_CDlgRemoteRecordSchedule.stringdata0))
        return static_cast<void*>(this);
    return QDialog::qt_metacast(_clname);
}

int CDlgRemoteRecordSchedule::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 4)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 4;
    }
    return _id;
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
