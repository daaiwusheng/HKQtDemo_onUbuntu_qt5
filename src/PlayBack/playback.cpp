 ...
 *  Return          :  	none
************************************************************************/
void PlayBack::insertrow(NET_DVR_FINDDATA_V50 &dvrfile)
{
    int row ;
	...
	QString month;
	month = QString::number(dvrfile.struStartTime.byMonth);
	if (dvrfile.struStartTime.byMonth <=9)
	{
		month.prepend("0");
	}
	QString day;
	day = QString::number(dvrfile.struStartTime.byDay);
	if (dvrfile.struStartTime.byDay <= 9)
	{
		day.prepend("0");
	}
	QString hour;
	hour = QString::number(dvrfile.struStartTime.byHour);
	if (dvrfile.struStartTime.byHour <= 9)
	{
		hour.prepend("0");
	}
	QString minute;
	minute = QString::number(dvrfile.struStartTime.byMinute);
	if (dvrfile.struStartTime.byMinute <= 9)
	{
		minute.prepend("0");
	}
	QString sec;
	sec = QString::number(dvrfile.struStartTime.bySecond);
	if (dvrfile.struStartTime.bySecond <= 9)
	{
		sec.prepend("0");
	}
    QTableWidgetItem *beginTimeItem = new QTableWidgetItem( 
        tr("%1%2%3%4%5%6").arg(int(dvrfile.struStartTime.wYear)
	...
	month =QString::number(dvrfile.struStopTime.byMonth);
	if (dvrfile.struStopTime.byMonth <= 9)
	{
		month.prepend("0");
	}
	day = QString::number(dvrfile.struStopTime.byDay);
	if (dvrfile.struStopTime.byDay <= 9)
	{
		day.prepend("0");
	}
	hour = QString::number(dvrfile.struStopTime.byHour);
	if (dvrfile.struStopTime.byHour <= 9)
	{
		hour.prepend("0");
	}
	minute = QString::number(dvrfile.struStopTime.byMinute);
	if (dvrfile.struStopTime.byMinute <= 9)
	{
		minute.prepend("0");
	}
	sec = QString::number(dvrfile.struStopTime.bySecond);
	if (dvrfile.struStopTime.bySecond <= 9)
	{
		sec.prepend("0");
	}
    QTableWidgetItem *endTimeItem = new QTableWidgetItem( 
        tr("%1%2%3%4%5%6").arg(int(dvrfile.struStopTime.wYear)).arg(
...
void PlayBack::on_pushButton_1_search_clicked()
{
	...
	m_pbpDvrFileV50 = (LPNET_DVR_FILECOND_V50)malloc(sizeof( NET_DVR_FILECOND_V50));

    if (m_pbpDvrFileV50 == NULL)
    {
        QMessageBox::information(this,tr("PlayBack SearchFile"),
            tr("Malloc Error in PlayBack!"));
        return;
    }
    memset(m_pbpDvrFileV50,0x0,sizeof(NET_DVR_FILECOND_V50));

    //give the variant prepare value
    m_pbpDvrFileV50->struStartTime.wYear = m_pbremotestarttime.dwYear;
	m_pbpDvrFileV50->struStartTime.byMonth = m_pbremotestarttime.dwMonth; 
	m_pbpDvrFileV50->struStartTime.byDay = m_pbremotestarttime.dwDay;
	m_pbpDvrFileV50->struStartTime.byHour = m_pbremotestarttime.dwHour;
	m_pbpDvrFileV50->struStartTime.byMinute = m_pbremotestarttime.dwMinute;
	m_pbpDvrFileV50->struStartTime.bySecond = m_pbremotestarttime.dwSecond;

    m_pbpDvrFileV50->struStopTime.wYear = m_pbremotestoptime.dwYear;
	m_pbpDvrFileV50->struStopTime.byMonth = m_pbremotestoptime.dwMonth; 
	m_pbpDvrFileV50->struStopTime.byDay = m_pbremotestoptime.dwDay;
	m_pbpDvrFileV50->struStopTime.byHour = m_pbremotestoptime.dwHour;
	m_pbpDvrFileV50->struStopTime.byMinute = m_pbremotestoptime.dwMinute;
	m_pbpDvrFileV50->struStopTime.bySecond = m_pbremotestoptime.dwSecond;


    m_pbpDvrFileV50->struStreamID.dwChannel = (*m_pbCurrDevInfo).byStartChan + (*m_pbCurrChannel) -1;
	if (*m_pbCurrChannel == 0)
	{
		m_pbpDvrFileV50->struStreamID.dwChannel = (*m_pbCurrDevInfo).byStartChan;
	}
    m_pbpDvrFileV50->byIsLocked = m_pbfileattr;
    m_pbpDvrFileV50->dwFileType = m_pbfiletype;
    ...
    hFile = NET_DVR_FindFile_V50(*m_pbUserID, m_pbpDvrFileV50);
    if (hFile < 0)
    {
        QMessageBox::information(this,tr(" NET_DVR_FindFile_V50 "), 
	...
	else
	{
		...
		while (bGetFile)
	    {
	        memset(&m_pbFileDataV50, 0, sizeof(NET_DVR_FINDDATA_V50));
	        ret = NET_DVR_FindNextFile_V50(hFile, &m_pbFileDataV50);
	        switch (ret)
	        {
	            case NET_DVR_FILE_SUCCESS:
	                insertrow(m_pbFileDataV50);
	                break;

	            case NET_DVR_FILE_NOFIND:
	                QMessageBox::information(this,tr(" NET_DVR_FindNextFile_V50"), 
	        			tr("No file find!"));
...
void PlayBack::on_pushButton_playback_start_3_clicked()
{
    ...
    memcpy(&struVodPara.struBeginTime, &m_pbplaystarttime, sizeof(NET_DVR_TIME));
    memcpy(&struVodPara.struEndTime, &m_pbplaystoptime, sizeof(NET_DVR_TIME));
    struVodPara.hWnd = playWnd;
    struVodPara.struIDInfo.dwChannel = *m_pbCurrChannel;


#if (defined(_WIN32)) || (defined(__APPLE__))
        //m_pbhandle = NET_DVR_PlayBackByTime(*m_pbUserID,*m_pbCurrChannel, &m_pbplaystarttime, &m_pbplaystoptime, playWnd);
		m_pbhandle = NET_DVR_PlayBackByTime_V40(*m_pbUserID, &struVodPara);
#elif defined(__linux__)
        //m_pbhandle = NET_DVR_PlayBackByTime(*m_pbUserID,*m_pbCurrChannel, &m_pbplaystarttime, &m_pbplaystoptime, playWnd);
		m_pbhandle = NET_DVR_PlayBackByTime_V40(*m_pbUserID, &struVodPara);
#endif
        ...
        if (m_pbhandle<0)
        {
            QMessageBox::information(this,tr("NET_DVR_PlayBackByTime_V40 failed"),
                tr("SDK_Last_Error =%1").arg(NET_DVR_GetLastError()));
                    return;
        }
        playbackcontrol(NET_DVR_PLAYSTART);
